...
sensor_step_down = 9  # Step to decrease brightness
dimming_delay_time_sensor = 2
timeout_increase_sensor = 1  # Initial timeout in seconds before dimming starts
...
@service
def increase_timeout_sensor(timer):
    global interrupted_sensor
    current_timer_value = int(get_seconds(timer))

# hass.states.get("timer.entrance_timer").state
    new_time=format_time(current_timer_value + timeout_increase_sensor)
...
@service
def start_dimming_sensor(timer):
    global interrupted_sensor
    # interrupted_sensor = False
    # Start dimming the light
    brightness = 0
    
    try:
        brightness = int(hass.states.get('input_text.result').state)
    except ValueError:
        brightness = 0
    ...
    new_brightness = 50 if brightness > 50 else max(0, brightness - sensor_step_down)
...
def ciao():
    ...
    if (brightness > 0):
        ...
        duration = timer_seconds_left if timer_state != 'active' and timer_seconds_left != 0 else dimming_delay_time_sensor
...
def handle_motion_sensor_triggered():
    ...
    increase_timeout_sensor('timer.entrance_timer')
...
def handle_motion_sensor_released():
    ...
    timer_state = hass.states.get(timer).state
    timer_seconds_left = int(get_seconds(timer))
    duration = timer_seconds_left if timer_state != 'active' and timer_seconds_left != 0 else dimming_delay_time_btn
    set_timer(timer, duration)